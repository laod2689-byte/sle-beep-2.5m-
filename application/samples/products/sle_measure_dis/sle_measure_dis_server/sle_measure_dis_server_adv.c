...
errcode_t measure_dis_sle_set_adv(uint8_t announce_id, uint8_t *addr)
{
    ...
    ret = measure_dis_set_default_announce_param(announce_id, &sle_addr);
    if (ret != ERRCODE_SLE_SUCCESS) {
        return ret;
    }

    return measure_dis_set_default_announce_data(announce_id);
}
